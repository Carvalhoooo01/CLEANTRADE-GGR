...
export async function PATCH(request: Request) {
  try {
    ...
    const data: { status?: string; quantidade?: number; vendidos?: number } = {};
...
export async function DELETE(request: Request) {
  try {
    ...
    // Bloqueia exclusao se estiver ativo no marketplace
    if (lote.status === "ativo") {
      return NextResponse.json(
        { error: "Pause o lote antes de remover. Lotes ativos nao podem ser excluidos." },
        { status: 400 }
      );
    }

    // Bloqueia exclusao de lotes CarbonCorp
    if (lote.certificadora === "CarbonCorp") {
      return NextResponse.json(
        { error: "Lotes CarbonCorp sao gerenciados automaticamente e nao podem ser excluidos." },
        { status: 400 }
      );
    }
