...
export async function POST(request: Request) {
  try {
    const { userId, type, cert, amount, price, total, status, txid, pixPayload } = await request.json();
    ...
    return NextResponse.json({ error: "Erro ao salvar transacao" }, { status: 500 });
  }
}
...
function formatTransacao(t: any) {
