  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: 18, boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        <SectionHeader title="Evolução de Compras" action={<Btn small variant="ghost" onClick={() => { downloadCSV(MONTHLY_DATA, "historico.csv"); showToast("CSV exportado! 📥"); }}>{Icons.download} CSV</Btn>} />
        <ResponsiveContainer width="100%" height={90}>
          <AreaChart data={MONTHLY_DATA}>
            <defs><linearGradient id="gH" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#16a34a" stopOpacity={0.15} /><stop offset="95%" stopColor="#16a34a" stopOpacity={0} /></linearGradient></defs>
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: 18, boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        <SectionHeader title="Todas as Transações" action={<Btn small variant="outline" onClick={() => showToast("Relatório gerado! 📄")}>{Icons.download} Relatório</Btn>} />
        {transactions.map((t) => (
