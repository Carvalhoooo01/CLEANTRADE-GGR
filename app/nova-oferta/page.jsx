...
export default function NovaOfertaPage() {
  const { setLotes, publicarLote, showToast } = useApp();
  ...
  const handleSubmit = () => {
    if (!nome || !qty || !preco) { showToast("Preencha todos os campos obrigatórios", "error"); return; }
    const novoLote = { id: Date.now(), name: nome, type: tipo, cert, quantidade: parseInt(qty), preco: parseFloat(preco), status: "ativo", vendidos: 0, receita: 0, descricao: desc || `Crédito ${tipo}`, data: new Date().toLocaleDateString("pt-BR") };
    setLotes((ls) => [...ls, novoLote]);
    if (pub) publicarLote(novoLote);
    showToast(pub ? "Lote criado e publicado no Marketplace! 🎉" : "Lote salvo como rascunho!", "success");
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: 22, boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <input value={nome} onChange={(e) => setNome(e.target.value)} style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 12px", fontSize: 13, outline: "none", fontFamily: "inherit", color: "#111827" }} />
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div>
              ...
              <input type="number" value={qty} onChange={(e) => setQty(e.target.value)} style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 12px", fontSize: 13, outline: "none", fontFamily: "inherit" }} />
            </div>
            <div>
              <label style={{ fontSize: 12, fontWeight: 600, color: "#374151", display: "block", marginBottom: 6 }}>Preço por tCO₂ (R$) *</label>
              <input type="number" value={preco} onChange={(e) => setPreco(e.target.value)} style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 12px", fontSize: 13, outline: "none", fontFamily: "inherit" }} />
            </div>
          </div>

          <div>
            <label style={{ fontSize: 12, fontWeight: 600, color: "#374151", display: "block", marginBottom: 6 }}>Descrição</label>
            <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={2} placeholder="Descreva o projeto..." style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 8, padding: "8px 12px", fontSize: 13, outline: "none", fontFamily: "inherit", resize: "vertical" }} />
          </div>

          <div style={{ background: "#f0f9ff", borderRadius: 10, padding: "10px 14px", border: "1px solid #bae6fd", fontSize: 12, color: "#0369a1" }}>
            💡 <strong>Créditos são indivisíveis</strong> — cada unidade = 1 tCO₂ com ID único no Verra Registry. Quantidade deve ser inteira.
          </div>
          ...
          <Btn style={{ justifyContent: "center", width: "100%", background: "linear-gradient(135deg,#0ea5e9,#38bdf8)", boxShadow: "0 3px 12px rgba(14,165,233,0.3)" }} onClick={handleSubmit}>
            {Icons.check} {pub ? "Criar e Publicar" : "Salvar Rascunho"}
          </Btn>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ background: "#fff", borderRadius: 14, padding: 18, boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
          ...
          <div style={{ border: "1px solid #e5e7eb", borderRadius: 10, padding: 14 }}>
            ...
            {[["Certificadora", cert], ["Disponível", fmtCO2(parseInt(qty) || 0)], ["Preço", fmt(parseFloat(preco) || 0) + "/tCO₂"], ["IDs Verra", `${parseInt(qty) || 0} créditos únicos`]].map(([k, v]) => (
